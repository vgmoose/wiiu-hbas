...
Zip::Zip(const char * zipPath) {
	...
	if(fileToZip == NULL) log_printf("Error Opening: %s for zipping files!\n",zipPath);
}
...
int Zip::AddFile(const char * internalPath, const char * path) {
	...
	log_printf("Adding %s to zip, under path %s\n",path,internalPath);
	int code = Add(path);
...
int UnZip::Extract(const char * path, unz_file_info_s * fileInfo) {
	log_printf("Extract: Path is %s", path);
	...
	//Somehow folder creation was wrong.
	char* slash_ptr = strrchr(path, '/');
	u32 slash_pos = (u32)slash_ptr - (u32)path; // A bit hacky, but it should work...

	char* folder_path = (char*)malloc(slash_pos + 1);
	memcpy(folder_path, path, slash_pos);
	folder_path[slash_pos] = 0;

	log_printf("Extract: Creating folder %s", folder_path);
	CreateSubfolder(folder_path);
